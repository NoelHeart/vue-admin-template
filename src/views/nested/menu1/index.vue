<template>
  <div class="app-container">
    <div class="filter-container">
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="getEmChart">昨天总体用电量变化图</el-button>
    </div>
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column min-width="200px" label="日期">
        <template slot-scope="scope">
          <span>{{ scope.row.dateDay }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="详情" :visible.sync="dialogFormVisible1">
      <el-form ref="dataForm1" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="日期" prop="scale" >
          <el-input v-model="temp.dateDay" :disabled="true"/>
        </el-form-item>
    ...
    <el-dialog width="100%"  :visible.sync="dialogFormVisible3">
      <yesterday-em-chart width="100%"/>
    </el-dialog>
  </div>
</template>

<script>
  import yesterdayEmChart from '../components/yesterdayEmChart'
  import { getEmDailyRecordList, deleteEmDailyRecord } from '@/api/compute'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getEmDailyRecordList(requestData).then(response => {
      ...
      handleDelete(row){
        ...
        this.$confirm('此操作将删除该条记录, 是否继续?', '提示', {
          ...
          deleteEmDailyRecord(this.temp).then(() => {
