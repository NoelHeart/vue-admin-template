    ...
    <el-dialog title="创建" :visible.sync="dialogFormVisible1">
      <el-form ref="dataForm1" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="设备类型">
          <el-input :disabled="true" placeholder="电表"/>
        </el-form-item>
      </el-form>
...
<script>
  import { getElectricList, createElectricMeter, updateElectricMeter, deleteElectricMeter } from '@/api/honeycomb'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getElectricList(requestData).then(response => {
      ...
      createData(){
        this.$refs['dataForm1'].validate((valid) => {
          if (valid) {
            createElectricMeter(this.temp).then(() => {
      ...
      updateData(){
        this.$refs['dataForm2'].validate((valid) => {
          if (valid) {
            ...
            updateElectricMeter(tempData).then(() => {
      ...
      handleDelete(row){
        this.temp = Object.assign({}, row) // copy obj
        this.$confirm('此操作将删除电表'+this.temp.deviceId+', 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          deleteElectricMeter(this.temp).then(() => {
            for (const v of this.list) {
              if (v.id === this.temp.id) {
                const index = this.list.indexOf(v)
