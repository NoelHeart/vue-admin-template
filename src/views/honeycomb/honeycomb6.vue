<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column min-width="300px" label="水表设备">
        <template slot-scope="scope">
    ...
    <el-dialog title="创建水表部署" :visible.sync="dialogFormVisible1">
      <el-form ref="dataForm1" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-select v-model="temp.deviceId" filterable placeholder="水表设备" clearable style="width: 300px" class="filter-item">
    ...
    <el-dialog title="更新水表部署" :visible.sync="dialogFormVisible2">
...
<script>
  import { getWmApartList, createWmApart, updateWmApart, deleteWmApart, WmList } from '@/api/honeycomb'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getWmApartList(requestData).then(response => {
      ...
      handleDelete(row){
        this.temp = Object.assign({}, row) // copy obj
        this.$confirm('此操作将删除该水表部署'+this.temp.deviceId+', 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          deleteWmApart(this.temp).then(() => {
