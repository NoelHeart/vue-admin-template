<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column min-width="100px" label="事件ID">
      ...
      <el-table-column min-width="200px" label="数据内容">
        <template slot-scope="scope">
      ...
      <el-table-column min-width="200px" label="数据源">
        <template slot-scope="scope">
          <span>电表 {{ scope.row.deviceId}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="200px" label="数据时间" >
        <template slot-scope="scope">
          <span>{{ scope.row.dataTime }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="200px" label="更新时间" >
        <template slot-scope="scope">
          <span>{{ scope.row.updateTime}}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="200px" label="所属公司" >
        <template slot-scope="scope">
    ...
    <el-dialog title="创建电表事件记录" :visible.sync="dialogFormVisible1">
      <el-form ref="dataForm1" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="事件名称" prop="eventname">
          <el-input v-model="temp.eventname"/>
        </el-form-item>
        <el-form-item label="数据项" prop="cid">
          <el-input v-model="temp.cid"/>
        </el-form-item>
        <el-form-item label="数据内容" prop="data">
    ...
    <el-dialog title="更新电表事件记录" :visible.sync="dialogFormVisible2">
...
<script>
  import { getEmEventList, createEmEvent, updateEvent, deleteEvent, EmList } from '@/api/honeycomb'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getEmEventList(requestData).then(response => {
