...
<script>
  import { getHxscreenCameraList, addHxscreenCamera, deleteHxscreenCamera } from '@/api/hxscreen'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getHxscreenCameraList(requestData).then(response => {
      ...
      createData(){
        this.$refs['dataForm1'].validate((valid) => {
          if (valid) {
            addHxscreenCamera(this.temp).then(() => {
              this.getList()
      ...
      handleDelete(row){
        ...
        this.$confirm('此操作将删除该条记录, 是否继续?', '提示', {
          ...
          deleteHxscreenCamera(this.temp).then(() => {
