  ...
  <div class="app-container">
    ...
    >
      <el-table-column align="center" label="摄像头ID" >
        <template slot-scope="scope">
          <span>{{ scope.row.cameraIdId }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog title="创建" :visible.sync="dialogFormVisible1">
      <el-form ref="dataForm1" :rules="rules" :model="temp" label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="摄像头ID" prop="cameraIdId">
          <el-input v-model="temp.cameraIdId"/>
        </el-form-item>
...
<script>
  import { getHxscreenWindow3List, addHxscreenWindow3, deleteHxscreenWindow3 } from '@/api/hxscreen'
  ...
  export default {
    ...
    data(){
      return{
        ...
        temp: {
          id:undefined,
          cameraIdId:undefined,
          address:undefined
        },
        dialogFormVisible1:false,
        rules: {
          cameraIdId:[{required:true}],
    ...
    methods: {
      getList() {
        ...
        getHxscreenWindow3List(requestData).then(response => {
      ...
      createData(){
        this.$refs['dataForm1'].validate((valid) => {
          if (valid) {
            addHxscreenWindow3(this.temp).then(() => {
              this.getList()
      ...
      handleDelete(row){
        ...
        this.$confirm('此操作将删除该条记录, 是否继续?', '提示', {
          ...
          deleteHxscreenWindow3(this.temp).then(() => {
