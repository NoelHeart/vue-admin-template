...
<script>
  import { getHxscreenWindow4List, addHxscreenWindow4, deleteHxscreenWindow4 } from '@/api/hxscreen'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getHxscreenWindow4List(requestData).then(response => {
      ...
      createData(){
        this.$refs['dataForm1'].validate((valid) => {
          if (valid) {
            addHxscreenWindow4(this.temp).then(() => {
              this.getList()
      ...
      handleDelete(row){
        ...
        this.$confirm('此操作将删除该条记录, 是否继续?', '提示', {
          ...
          deleteHxscreenWindow4(this.temp).then(() => {
