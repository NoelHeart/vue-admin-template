...
<script>
  import { getHxscreenWindow1List, addHxscreenWindow1, deleteHxscreenWindow1 } from '@/api/hxscreen'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getHxscreenWindow1List(requestData).then(response => {
      ...
      createData(){
        this.$refs['dataForm1'].validate((valid) => {
          if (valid) {
            addHxscreenWindow1(this.temp).then(() => {
              this.getList()
      ...
      handleDelete(row){
        ...
        this.$confirm('此操作将删除该条记录, 是否继续?', '提示', {
          ...
          deleteHxscreenWindow1(this.temp).then(() => {
