...
<script>
  import { getHxscreenWaterDailyList, addHxscreenWaterDaily, deleteHxscreenWaterDaily } from '@/api/hxscreen'
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        getHxscreenWaterDailyList(requestData).then(response => {
      ...
      createData(){
        this.$refs['dataForm1'].validate((valid) => {
          if (valid) {
            addHxscreenWaterDaily(this.temp).then(() => {
              this.getList()
      ...
      handleDelete(row){
        ...
        this.$confirm('此操作将删除该条记录, 是否继续?', '提示', {
          ...
          deleteHxscreenWaterDaily(this.temp).then(() => {
